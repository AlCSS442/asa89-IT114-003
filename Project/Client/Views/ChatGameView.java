...
public class ChatGameView extends JPanel implements IRoomEvents, IPhaseEvent {
    ...
    public ChatGameView(ICardControls controls) {
        ...
        // Creating the User List Panel
       // createUserListPanel();

        // Start with chat only
        showChatOnlyView();

        // Registering for Server Callbacks
    ...
    @Override
    public void onUserListUpdate(List<UserInfo> users) {

        SwingUtilities.invokeLater(() -> {
            userListModel.clear();

            // Sort users by turn order
            users.sort(Comparator.comparingInt(UserInfo::getTurnOrder));
